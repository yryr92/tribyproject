...
<%
	...
	String title = (String)request.getAttribute("searchNoticeTitle");
        ...
        <div class="body">
            ...
            <div class="contentWrap">
                ...
                <div class="tagWrap">
                    ...
                    <div class="searchNoticeForm">
                       <form action="<%=request.getContextPath() %>/searchNotice.ad?" method="post">
						  <span>제목:</span>
                          <input type="search" name="searchNoticeTitle" id="searchNoticeTitle" value="<%=title %>">
                ...
                <div class="pagingArea" align="center">
                	<button onclick="location.href='<%=request.getContextPath()%>/searchNotice.ad?searchNoticeTitle=<%=title%>&currentPage=1';">&lt;&lt;</button>
                	<%if(currentPage ==1) { %>
                		<button disabled>&lt;</button>
                	<%}else { %>
                		<button onclick="location.href='<%=request.getContextPath()%>/searchNotice.ad?searchNoticeTitle=<%=title%>&currentPage=<%=currentPage-1%>';">&lt;</button>
                	<%} %>
                	...
                	<%}else { %>
                		<button onclick="location.href='<%=request.getContextPath()%>/searchNotice.ad?searchNoticeTitle=<%=title%>&currentPage=<%=p%>';"><%=p %></button>
                	<%} %>
                	<%} %>
                	...
                	<%if(currentPage == maxPage) { %>
                		<button disabled>&gt;</button>
                	<%}else { %>
                		<button onclick="location.href='<%=request.getContextPath()%>/searchNotice.ad?searchNoticeTitle=<%=title%>&currenPage=<%=currentPage+1%>';">&gt;</button>
                	<%} %>
                	<button onclick="location.href='<%=request.getContextPath() %>/searchNotice.ad?searchNoticeTitle=<%=title%>&currentPage=<%=maxPage %>';">&gt;&gt;</button>
                </div>
            </div>
        </div>    
    </div>
