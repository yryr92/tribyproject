        ...
        <div class="body">
            ...
            <div class="contentWrap">
                ...
                <div class="tagWrap">
                    ...
                    <div class="write"><button type="button" onclick="location.href='<%=request.getContextPath()%>/insertNoticeForm.ad';"><div class="btn" id="write_notice">공지 작성</div></button></div></div>
                    <div class="searchNoticeForm">
                       <form action="<%=request.getContextPath() %>/searchNotice.ad?" method="post">
                          ...
                          <input type="submit" value="전체검색" id="searchNoticeInput" onclick="">
                       </form>
                    </div>
                ...
                <div class="pagingArea" align="center">
                	<button onclick="location.href='<%=request.getContextPath()%>/notice.ad?currentPage=1';">&lt;&lt;</button>
                	<%if(currentPage ==1) { %>
                		<button disabled>&lt;</button>
                	<%}else { %>
                		<button onclick="location.href='<%=request.getContextPath()%>/notice.ad?currentPage=<%=currentPage-1%>';">&lt;</button>
                	<%} %>
                	...
                	<%}else { %>
                		<button onclick="location.href='<%=request.getContextPath()%>/notice.ad?currentPage=<%=p%>';"><%=p %></button>
                	<%} %>
                	<%} %>
                	...
                	<%if(currentPage == maxPage) { %>
                		<button disabled>&gt;</button>
                	<%}else { %>
                		<button onclick="location.href='<%=request.getContextPath()%>/notice.ad?currenPage=<%=currentPage+1%>';">&gt;</button>
                	<%} %>
                	<button onclick="location.href='<%=request.getContextPath() %>/notice.ad?currentPage=<%=maxPage %>';">&gt;&gt;</button>
                </div>
            </div>
        </div>    
    </div>
